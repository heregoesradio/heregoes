...
### Motivation
At off-nadir viewing angles, the apparent positions of Earth features like cloud or high terrain are displaced in satellite imagery. Cloud height can be estimated from radiance and used to derive the true geodetic coordinates of cloud pixels[^1]. Orthorectification is the inverse problem, where terrain height and coordinates are known, but the pixel locations are not[^2].

In 2022, Pestana and Lundquist[^3] demonstrated that rapid orthorectification of ABI imagery can be achieved in Python by interpolating ABI pixels onto a digital elevation model (DEM). However, ABI L1b radiance is already resampled from detector counts through ground system processing[^4], which complicates and adds uncertainty to sub-pixel retrievals. If we instead interpolate the underlying coordinate system, the ABI Fixed Grid[^5], then we can accurately navigate pixels *in situ* and preserve the original sampling of the L1b image (Figure 1).
...
By reversing the direction of parallax correction, `heregoes` can also orthorectify the ABI image itself (Figure 2) while maintaining the same geostationary projection.
...
[^5]: https://www.goes-r.gov/users/docs/PUG-GRB-vol4.pdf
[^6]: https://doi.org/10.1029/2019EA000658
