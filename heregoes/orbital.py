...
def gmst(jdays2000):
    ...
    theta = 67310.54841 + ut1 * (876600 * 3600.0 + 8640184.812866 + ut1 *
                                 (0.093104 - ut1 * 6.2 * 10.0e-6))
...
def sun_ra_dec(jdays2000):
    ...
    eps = np.deg2rad(23.0 + 26.0 / 60.0 + 21.448 / 3600.0 -
                     (46.8150 * jdate + 0.00059 * jdate * jdate -
                      0.001813 * jdate * jdate * jdate) / 3600.0)
...
def sun_ecliptic_longitude(jdays2000):
    ...
    d_l = ((1.914600 - 0.004817 * jdate - 0.000014 * jdate * jdate) * np.sin(m_a) +
           (0.019993 - 0.000101 * jdate) * np.sin(2.0 * m_a) + 0.000290 * np.sin(3.0 * m_a))
    # true longitude, deg
    l__ = l_0 + d_l
...
def get_alt_az(jdays2000, lon, lat):
    ...
    alt = np.arcsin(np.sin(lat) * np.sin(dec) +
                      np.cos(lat) * np.cos(dec) * np.cos(h__))

    az = np.arctan2(-np.sin(h__), (np.cos(lat) * np.tan(dec) -
...
def get_observer_look(sat_lon, sat_lat, sat_alt, jdays2000, lon, lat, alt):
    ...
    (opos_x, opos_y, opos_z), (ovel_x, ovel_y, ovel_z) = \
        observer_position(jdays2000, lon, lat, alt)
    ...
    top_z_divided_by_rg_ = np.clip(top_z_divided_by_rg_, np.nanmin(top_z_divided_by_rg_), 1.0)
